...
| Unit | PR / SHA | State |
| --- | --- | --- |
| **THE EXACT DOCUMENT A CLIENT RECEIVED, AND WHAT THEY SIGNED** (owner brief 2026-09-07; D1–D14 in `case-portal/SENT-DOCUMENTS.md`). `send_log` recorded THAT a send happened and never WHAT WENT, so "which exact rate sheet and terms did this client receive and sign?" could only be answered by re-rendering today's template over yesterday's send. **`sent_document` stores the rendered BYTES** — subject and both body parts as handed to the provider, plus their SHA-256 — and **the display name was REFUSED as the version**, because two documents with different non-refundable amounts wear the same one. **`send_log` is untouched**: no row rewritten, no CHECK widened, four additive tables, `kind` with no CHECK. **ACCEPTANCE IS LINKED, NEVER INFERRED** — written only from the token the door itself carried; a same-name, same-address submission with no token links to nothing, and a token resolving to no document writes no link, because that would manufacture evidence a client signed something never sent. **No acknowledgement checkbox was added and none may be** — what was missing was never consent, it was the link. **One attempt key per send, claimed BEFORE the provider**: `done` emails nobody, `indeterminate` is refused BY NAME rather than sent again on a guess. **CLIENT SENT / OWNER COPY FAILED is said out loud** (the Worker always knew; the page read neither field) and the resend **cannot email the client, structurally**. Home → Prepare & Send is ONE TAP with the type control on the screen; two presets plus Custom with `RETAINER_STANDARD` and the backend unmoved; seven Home cards, both timestamp tools still primary. **What the suites caught, all mine, none relaxed:** a select registered in the click chain that drew and did nothing; "Rate sheet"/"Retainer" labels reaching an investigator; the office-copy strip living inside the open-card branch so it would never have drawn on the new one-tap path; three build guards firing on my own prose (a dollar figure, an internal product name in public source, a `.cr-m` class collision); and four harness errors of mine that would have blamed the product. The four-preset pin — which once correctly refused me adding $1,000 on my own inference — follows the owner's own restatement of that decision. | #299 `ae1f3d2` | ✅ **DEPLOYED** — `deploy.yml` run #373 **success**, `deploy-portal.yml` run #134 **success**, both on `ae1f3d29`; **`portal-setup.yml` run #58 dispatched and SUCCESS**, schema applied. Suites at merge: worker **3871/0** · e2e **3448/0** · CEO gate **43 PASS/0 WARN/0 FAIL** · deploy guard **92/0** · intake **671/0** · visitor **49/0**. LIVE VERIFY **OPEN** for the owner |
| **THE HYBRID VISUAL PASS** (owner brief 2026-09-07 plus an approved art sheet; specifications in `case-portal/ART-ASSET-MANIFEST.md`). Seven primary Home actions become art cards; every operational screen stays exactly as plain as it was, and no business logic, schema, route or destination moved. **THE CORRECTION THE PASS RESTS ON:** `.uiart` painted `--f-ink` — a TEXT colour — under a dark scrim, ALWAYS, which is why the art system had been switched off; with no photographs it made ten near-black slabs and crushed the family colours the cards exist to carry. A scrim stops a PHOTOGRAPH fighting white type, so with no photograph none is painted, and a card is its own ground at full strength until an asset lands. **MEASURED TWICE**, because contrast says nothing about sameness: white on the seven grounds 6.88-15.23 and the .88-alpha subtitle 5.74 at worst, AND held apart by CIE76 dE with the closest pair at 13.6 — a first draft's closest pair was 7.9 and read as one colour, which no luminance test could see. **TWO DEFECTS THIS UNIT PRODUCED:** `.uiact:hover` used the `background` SHORTHAND, which resets background-image too, so hovering an art card erased the ground AND the art and left white type at 1.06:1 on every desktop card — invisible to a screenshot with the pointer parked elsewhere, found by the painted-pixel measurement; and the icon was pinned top with the title bottom, opening a void across a square card, found by looking at the render. A third was caught by the audit: the strip renders from `shell()`, so the first build put seven dark cards on Cases, Intakes, Rate Sheets, Delivery and Settings — the doors stay, the TREATMENT is withdrawn. **ONE STRIP, TWO PROPORTIONS** — the desktop chip row is gone, four across and landscape there against two across and square on a phone, and the desktop set is the owner's seven, which also makes Settings -> My Portal true there for the first time; `dlabel` retires with it, the one wording this unit changes. **REFUSED ON EVIDENCE:** the sheet's bright-green action pill (white on it is 2.28:1, so it inverts to 6.88 both ways) and its "Capture & verify" subtitles, because this portal never claims verification. Height measured against master with the same fixture: 320 goes 747 -> 689 SHORTER, 390 goes 667 -> 801 for true 1:1 cards, desktop 76 -> 402. No artwork exists yet; `CARD_ART` is empty and every card draws its ground. | #301 `8abf7ca` | ✅ **DEPLOYED** — `deploy.yml` run #375 **success**, **`deploy-portal.yml` did not run and correctly did not** — it triggers on `case-portal/worker.js`, which this unit does not touch. **No schema change, so no `portal-setup.yml` dispatch.** Suites at merge: worker **3871/0** · e2e **3483/0** · CEO gate **43 PASS/0 WARN/0 FAIL** · deploy guard **92/0** · intake **671/0** · visitor **49/0**. LIVE VERIFY **OPEN** for the owner |
| **THE SEVEN HOME CARD PHOTOGRAPHS, INSTALLED** (owner's approved hybrid art pack, 2026-09-07; provenance and measurements in `case-portal/ART-ASSET-MANIFEST.md`). The art system built one unit earlier took the real artwork at exactly the cost it promised: **one asset file, one name in `CARD_ART`, one line in `deploy-manifest.txt` each, plus a single measured `object-position` nudge** — no layout change, no geometry change, no component rebuild, no route or destination moved. Sources 1254² → **1024² LANCZOS, WebP q82**, 33–62 KB each, **368 KB total**; q88 was rejected because the largest a card ever draws is 244×134, so the source is heavily downscaled and the extra 30% bought nothing visible. **THREE GATES HAVE TO AGREE OR A CARD BREAKS SILENTLY** — the name in `CARD_ART` or no URL is emitted, the file in `portal/cards/` or the request 404s and the card falls back to its ground with nothing saying so, the path in `deploy-manifest.txt` or it is simply not published; all three are asserted, including a REAL FETCH of every URL from the rendered page, and the manifest lines are named one by one because a directory entry is default-ALLOW inside it. **ONE CROP ADJUSTMENT, MEASURED:** `timestamp-photo` centred put its golden marble directly under the title at **4.54:1** — above the bar by 0.04, the razor-thin margin this project already rejected once — so it carries `center 65%` (swept: 0% 4.72, 35% 5.99, **65% 7.87**, 100% 5.16), and the other six needed nothing. **THE SCRIM WAS RETUNED AND ITS TWO STOPS WERE SWEPT SEPARATELY**, because they do different jobs: the bottom protects the text and cannot move (`.74` gives 4.65 on a phone, the thin-margin trap), while the top only dims where the text never reaches and sat at `.28` for no measured reason — halved to `.14` it makes each photograph about twice as bright where it is visible and costs almost nothing at the text. All seven now **5.01–10.23 painted** at every width, and the binding card differs by surface (Private Intake on desktop, Timestamp Photo on a phone), so the sweep must cover all seven. **No asset harmed readability and none needed replacing.** The wide signed-intake alert is pinned UNCHANGED — an `.uiart` that takes no photograph, because the pack has no wide asset and §5 keeps its hierarchy. Two assertions were inverted by the event they were waiting for: they required that no card claim a file and no scrim be painted, correct while `CARD_ART` was empty. | #303 `5dd0d2f` | ✅ **DEPLOYED** — `deploy.yml` run #377 on `5dd0d2f`, staging and the publish-safety guard green. **No schema change, so no `portal-setup.yml` dispatch**; `deploy-portal.yml` correctly did not run. Suites at merge: worker **3871/0** · e2e **3486/0** · CEO gate **43 PASS/0 WARN/0 FAIL** · deploy guard **92/0** · intake **671/0** · visitor **49/0**. LIVE VERIFY **OPEN** for the owner |
| **THE ART HOME CARDS BECOME TRUE DIRECT LAUNCHERS** (owner brief 2026-09-08). *"Tapping an art Home card can lead to another plain landing/quick-action layer before reaching the actual tool."* **Routing and duplicate-landing cleanup only** — no business logic, card order, Client Record, document/acceptance model, Retainer Paid, Close Case, CEO Bot, Assistant, bottom nav or timestamp functionality touched, and the artwork is untouched. `quickToolsHtml()` rendered from `shell()`, so tapping a card landed on a screen that **redrew the same grid above the tool**. **MEASURED WITH A CONTROL** — the same page and fixture with only this rule removed — and the cost was never taps: the duplicate grid is **667px tall at 390 and 747px at 320**, so New Intake, Private Intake and View Intakes each opened with their own heading at **y=790 on an 844-tall phone and y=870 on a 568-tall 320px screen**, the latter 302px BELOW THE FOLD. They open at **y=111** now. The Rate Sheet's form is a modal and always sat at the top (117/84, unchanged); its defect was the way back — closing left `TAB` on `sheets` with **zero art cards on screen and one more tap to Home**, against **seven and none** now, which is the owner's *"do not allow the art to disappear after Back"* stated as a number. `SHEET_WIZ.origin` restores where the wizard was opened from, so a send opened from a sheet card still returns to that list; `.amwrap` and `.amx` are two doors on one handler. **`homeTabs()` IS PER ROLE AND RETURNS A SET, AND BOTH HALVES WERE PAID FOR:** scoping to `"dashboard"` would have withdrawn the launcher from the field entirely (the 2026-09-04 incident verbatim), and `cases` ALONE was still half the answer — the phone's bottom nav has a Home of its own which for that role is **`today`**, so the first fix reintroduced the same incident one screen over. The second entry is **read off `MNAV`** rather than written down again. Measured, both branches, five screens each. **THREE E2E RUNS' WORTH OF FINDINGS WERE THE SUITE'S OWN:** `signIn` lands every section on Cases, so any section measuring the launcher was asserting something about *"the dashboard"* while standing somewhere else — one `filechooser` timeout aborted a run and 24 failures followed in the next; an audit of **all seventeen** strip-touching sections closes the class rather than the two that failed. | #305 `f50f7d3` | ✅ **DEPLOYED** — `deploy.yml` run **#379** (id 34183855070) on `f50f7d3`, **every step success** including *Prove the artifact is safe to publish*. **No schema change, so no `portal-setup.yml` dispatch**; `deploy-portal.yml` correctly did not run (last run #134 on `ae1f3d2`, and `case-portal/worker.js` is untouched here). Suites at merge: worker **3871/0** · e2e **3520/0** · CEO gate **43 PASS/0 WARN/0 FAIL** · deploy guard **92/0** · intake **671/0** · visitor **49/0**. LIVE VERIFY **OPEN** for the owner |
| **EVERY DIRECT-LAUNCH DESTINATION HAS A VISIBLE WAY BACK** (owner brief 2026-09-08). *"Some destination screens do not have a visible Back button and rely on the bottom Home nav instead."* **THE AUDIT SET THE SCOPE, NOT THE BRIEF'S LIST** — measured at 390 and 320 before anything was written: the Rate Sheet wizard's `.amx` was **30×28** and Active Surveillance's `.sv-x` **128×33**, both under this portal's own 44px floor; the New Intake chooser and View Intakes had **nothing at all**; Private Intake had only *Change type*, which goes to the chooser rather than Home; and **both timestamp tools were already correct** (`.vst-x`, 74×44) and are untouched, because the brief says not to duplicate a control that exists. **THE SET IS DERIVED FROM THE LAUNCHER, NEVER LISTED AGAIN:** `homeCardTabs()` reads `QT`, so a card added there gets its way back for free and a card whose destination changes takes its Back with it; `ACT_LANDS` names the two acts that land on a screen without navigating by tab id, beside the only thing that reads them; **the cards with their own root are deliberately absent**, because a `.pagebar` behind a full-screen tool is a control nobody can reach. It is the portal's EXISTING back — `.pagebar` + `.btn sm ghost`, the shape the case page and Clients & Firms already draw — measured **146×44, in flow, above the tool's own heading** at 390/320/1200, returning to the dashboard with seven art cards while **Home itself carries none**. **TWO FAMILIES, EACH INTERNALLY CONSISTENT, DELIBERATELY:** office screens take a top-LEFT pagebar back, the full-screen dialogs keep their labelled close top-RIGHT in their own sticky head; moving the second would restyle the field view and both timestamp tools. **THE PADDING PAID FOR THE FIELD FLOOR** — `.sv-head` 14px → 8px as `.sv-x` gained its minimum, so 14+33+14=61 became 8+44+8=60 and the sticky header did not move (measured both ways; at 320 it got 11px shorter). **NATIVE BACK CANNOT LOOP**, which is the absence of code rather than a guard: this page pushes no history for tabs, and a four-destination walk is asserted to add no entries. **AND THE CLASS-NAME CONTEST AGAIN, CAUGHT BY A CRASH:** the first run stopped at 360 on a strict-mode violation, and sweeping for that shape found the bigger one — **`.close` is a bare selector the suite clicks in SIXTEEN places**, so borrowing it for the new control put a second one on every screen it draws on. `.homeback` carries the floor and the layout itself; re-measured after the rename the geometry is identical, so the class contributed nothing but the collision. **NARROWED THE SAME DAY, ON THE OWNER'S OWN REVIEW** (#309 `50400ea`): *"keep the new Home-return behavior scoped to the seven primary Home launchers only ... do not add the new Back-to-Home pagebar to Cases or Reports & Packages."* `homeCardTabs()` reads **`QT_PHONE`** instead of both lists — the first build had derived from More as well and put the control on two ORDINARY sections, each of which already has a permanent door (Cases in the bottom navigation, Reports & Packages on the rail), so neither is a screen anyone can be stranded on. **Reading the list rather than naming three tabs keeps the derivation**: promote a card out of More and its destination gains a Back with it. The rule is now four screens — `sheets`, `newlead`, `leads` — and nothing else. Both halves are asserted, because a set that is merely SMALLER would satisfy the first assertion on its own, plus two live walks of Cases and Reports & Packages at 390 and 320 requiring no Back. | #307 `dabf13b` · #309 `50400ea` | ✅ **DEPLOYED** — `deploy.yml` run **#381** (id 34188517757) on `dabf13b`, **every step success** including *Prove the artifact is safe to publish*. **No schema change, so no `portal-setup.yml` dispatch**; `deploy-portal.yml` correctly did not run. Suites at merge: worker **3871/0** · e2e **3562/0** then **3571/0** after the narrowing · CEO gate **43 PASS/0 WARN/0 FAIL** · deploy guard **92/0** · intake **671/0** · visitor **49/0**. The narrowing deployed as `deploy.yml` run **#383** on `50400ea`, every step success. LIVE VERIFY **OPEN** for the owner |
| **THE CLIENT RECORD / DISPUTE PACKET** (owner brief 2026-09-08; D1–D12 in `case-portal/RECORD-PACKET.md`). One packet from a case — client dispute, chargeback file, the office's own records, an accountant — compiled from records the portal already preserved. `GET /cases/:no/record-packet` is admin-only, reads **nine tables that already existed**, **writes nothing and emails nobody**, and needs **no schema change and therefore no `portal-setup.yml` dispatch**. **THE SNAPSHOT IS THE PRODUCT:** the rate-sheet section returns `sent_document.body_text/body_html` — the bytes the provider was handed — and the block **calls no rate-sheet renderer**, enforced by a source pin beside "reaches no mail sender" and "contains no INSERT/UPDATE/DELETE", because a packet that COULD call the renderer could rebuild yesterday's document from today's figures and the output would look perfect. **TWO ABSENT STATES KEPT APART:** "no rate sheet was ever sent" is ordinary; an acceptance naming a document whose row is gone is the other thing, and only that one prints §12's exact sentence. **TWO DEFECTS THE SUITE FOUND IN MY OWN COMPOSER** — the intake's NOT AVAILABLE YET list was built by looking sideways from the value keys, so a payload carrying only a `<field>_status` reported nothing (the case INTAKE-NA exists for); and `sections.acceptance` ticked on a LINK, which would have put a tick over a document nobody signed. **AND ONE FACT THE TESTS TAUGHT ME ABOUT THE PRODUCT:** a PRE-CASE send has no `send_log.case_no`, so for the commonest path the DOCUMENT is the send record — asserting only `send_log` would have demanded the portal misattribute a send. **THE PDF GOES THROUGH THE ONE WRITER:** `pdfFromDoc` gained an OPTIONAL footer, every existing caller emits byte-identical output, and a comment of mine quoting the PDF header literal would have broken the guard counting it — caught before running. **AT 320 ALL FORTY RECORD ROWS WERE SCROLLING INSIDE THEMSELVES** while neither the page nor the document overflowed (the `.rs-l` defect one panel over); stacked below 560px at the END of the stylesheet, re-measured 0 of 22 overflowing at 320/390/1200. The anti-drift budget also caught four invented colour literals, one used three times, every one of which already had a token. **EMAIL IS DELIBERATELY NOT BUILT** (§14 makes it a *may*): the suite counts send-shaped verbs over every visible control on the screen and requires zero. One existing pin was **rewritten rather than loosened** — the timeline's source-order guard asserted its override followed the file's LAST 560px block, true only because that block happened to be last; it now asserts the property it protects. | #311 `52f0e1f` | ✅ **DEPLOYED** — `deploy.yml` run **#385** (id 34198232172) and `deploy-portal.yml` run **#135** (id 34198232167), both **success** on `52f0e1f`; the Worker deploy ran because this unit touches `case-portal/worker.js`. **No schema change, so no `portal-setup.yml` dispatch.** Suites at merge: worker **3918/0** · e2e **3606/0** · CEO gate **43 PASS/0 WARN/0 FAIL** · deploy guard **92/0** · intake **671/0** · visitor **49/0**. LIVE VERIFY **OPEN** for the owner |
| **THE PACKET'S GENERATION RECORD, ITS TIMELINE AND ITS SOURCE IDS** (owner brief 2026-09-08, the full-implementation round over #311; D13–D19 in `case-portal/RECORD-PACKET.md`). §5 record timeline, §15 RECORD INFORMATION, §16 the generation audit record, §18 the filename rule, §22 the case-history event, §24 recent packets and §25F multiple rate sheets. **ONE ADDITIVE TABLE, `packet_generation`**, written by exactly one `INSERT` and never updated or deleted — the owner's *"never overwrite historical packet-generation records"* as the absence of a writer rather than a guard, pinned from the source. `status` and the rest carry **no CHECK** (the Unit 7 rule), it is in `EXPECTED_TABLES`, `DEMO_SWEEP` and `INTAKE_EXEMPT` (audit history: it neither blocks an intake delete nor dies with one), and every read is `missingTables()`-guarded, so until the dispatch ran the packet composed exactly as before and the write route degraded by name. **§25F WAS THE ONE PRODUCT QUESTION AND THE RECORD ANSWERS IT:** with several rate sheets sent, the packet reproduces **the one the client ACCEPTED** — `acceptedDoc || delivered || sheetRows[0]`, resolved through the acceptance LINK that already exists and never by recency, because "the latest sheet" and "the sheet they signed" are the same document only until they are not. **§17 IS A SCOPE CALL, STATED ON THE SCREEN:** the PDF is **download-only** — the brief forbids inventing an external storage vendor, `sendMail` takes `{to,subject,text,html}` with no attachment support, and the existing evidence path is case EVIDENCE, which a packet compiled ABOUT the case is not. What IS kept for ever is the record of the generation: filename, SHA-256 of the produced bytes, the source document and submission ids, the section list and who generated it — so a packet handed to a bank a year ago can be identified byte for byte without the portal holding a copy. **§20 EMAIL IS THEREFORE NOT BUILT** and the suite still counts send-shaped verbs on the screen and requires zero. **THE DEFECT WORTH THE ROUND:** the write route read `src.doc_id` while the packet publishes `rate_sheet_doc_id`, so every generation record stored a **null document** — green in every shape assertion, wrong in the one column that makes the record identify anything, found only by reading a stored row back against what the composer publishes. A junk digest is refused **by shape** (`HEX64`) and answers `hash_recorded:false` rather than storing a lie — the `hash_origin` rule. **A TEST READ `e.kind` WHERE `tlEvent` SETS `type`**, so the new timeline arm looked absent while it was working perfectly; the arm is admin-only, `status='generated'` only, `TL.PACKETS:50`, `TL_RANK.packet:92`. **AND A SOURCE PIN FAILED ON MY OWN ROUTE** because I had placed the write inside the slice the read-only pin guards: the slice is bounded properly now and the write carries its own append-only companion pin, rather than either being loosened. The second door sits in **Billing & closing** beside the closeout — admin-gated, same `data-act`, no second implementation — and `crPacketName()` drops the stuttering `API-CASE-` prefix and counts a same-day `-2` suffix off the record itself. | #313 `b8c66b1` | ✅ **DEPLOYED** — `deploy.yml` run **#387** and `deploy-portal.yml` run **#136** (id 34249644511), both **success** on `b8c66b1`. **Schema change, so `portal-setup.yml` was dispatched: run #59** (id 34249779970), `workflow_dispatch` on `b8c66b1`, **conclusion success** — `packet_generation` applied. Suites at merge: worker **3950/0** · e2e **3624/0** · CEO gate **43 PASS/0 WARN/0 FAIL** · deploy guard **92/0** · intake **671/0** · visitor **49/0**. Measured after the new sections at 320/390/1200: **32 record rows, 0 overflowing**, page and document overflow zero, Generate PDF 46px, no page errors. LIVE VERIFY **OPEN** for the owner |
| **SIX LOCATION PAGES WERE ONE TEMPLATE WITH THE NAME SWAPPED** (owner question 2026-09-12: *"How is the search results doing? Anything we can beef up to get more clicks. We have a lot new such has insurance and legal"*; audit and baseline in `SEO.md`, root, internal). **THE HONEST ANSWER TO THE FIRST HALF IS THAT NOTHING IN THIS REPOSITORY CAN ANSWER IT** — the `/watch/` beacon keeps a referrer host in KV on a **600-second TTL**, a live view and not a history, so rankings, impressions and clicks need Google Search Console. `SEO.md` sets it up by **DNS TXT on purpose**: the HTML-file method would list `google<token>.html` in the manifest, and the stager **fails the build when a listed path is missing**, so a placeholder would freeze every deploy until the real file landed. **THE DUPLICATION WAS MEASURED, NOT ASSERTED:** stripped of tags, **28 of 33 sentences were byte-identical once the city name was substituted** and Roanoke/Danville shared **305 of 327 words** of vocabulary — six of the site's thirteen URLs competing to be the one Google does not filter. `PLACES` is dicts of FACTS now (roads, courts, adjacent localities, and a `ground` field for what working that locality actually involves — Bedford's rural sightlines, Roanoke's four jurisdictions in one valley, Charlottesville's city/county confusion, Danville's state line, Farmville's small-town conspicuousness, Lynchburg being where the office is) and the page is COMPOSED from them: ~700 → **~1,250 words**, unique main content **~6% → 44%**. **Every field is a verifiable geographic or civic fact or a statement about what this firm does** — no case volume, no named clients, no unevidenced work, because the public-page rule governs this like everything else. **THE FIRST METRIC WAS MISLEADING AND WAS REPLACED RATHER THAN CHASED:** counting identical sentences across the whole document counts the header, the footer, the Virginia adultery-law explanation and the ethics statement, two of which are compliance text it would be WRONG to vary; measured that way the number moved only 85% → 73% and the temptation was to pad. Stripping header and footer, the same work reads 6% → 44%. **INSURANCE AND LEGAL HAD NO LOCAL FOOTPRINT AT ALL** — every city page sold cheating and surveillance — and each now carries its own section routed to `?assignment=insurance` and `?assignment=legal`, never a bare or wrong door. **LEGAL WAS THE WEAKEST-LINKED PAGE ON THE SITE** despite being a primary business line: linked from none of the six city pages (all six linked Insurance), **no `BreadcrumbList` while every sibling had one**, sitemap priority `0.5/yearly` — the vendor sub-page's — and an H1 carrying neither "legal", "law firm", "attorney" nor "Virginia"; all four corrected, the original line kept as the standfirst. **FIVE DESCRIPTIONS WERE TRUNCATING**, insurance at 230 and legal at 220 against ~155–160, so **the phone number ending both never appeared in the result**. The hub — 221 words and an `ItemList`, on the URL aimed at *"private investigator near me"* — is 654 words with `ProfessionalService`, `BreadcrumbList` and `FAQPage`. **`lastmod` IS A HAND-EDITED CONSTANT AND THE REASON IS A DELAYED BUILD BREAK:** a git date moves the moment the file is committed, so the sitemap generated before the commit stops matching the one generated after it — the byte-for-byte guard would have passed on the day it shipped and failed the next. **THREE NON-LEVERS RECORDED SO THEY ARE NOT PAID FOR TWICE:** review stars (self-serving `LocalBusiness` review markup dropped by Google in 2019 — no stars, manual-action risk), more FAQ schema (restricted to government and health sites in 2023), and sitemap `priority`/`changefreq` (ignored). The biggest lever is not in this repository: the map pack, via Google Business Profile. | #315 `b9c8e39` | ✅ **DEPLOYED** — `deploy.yml` run **#389** (id 34726763331) on `b9c8e39`, **every step success** including *Prove the artifact is safe to publish*, and `build-locations.yml` run **#6** success with **no regeneration commit**, which is the proof the committed HTML already matched its generator. No schema change and no Worker touched, so no `portal-setup.yml` dispatch and `deploy-portal.yml` correctly did not run. Suites at merge: deploy guard **92/0** · intake **671/0** · visitor **49/0**; portal, worker, CEO-gate untouched by this unit (`git diff` over `portal/ case-portal/ visitor-alerts/ intake/` is empty). **The live GET could not be made from this session** — the environment's network policy denies outbound to the domain (403 on CONNECT) — so the deploy pipeline is the evidence and the page check is the owner's. LIVE VERIFY **OPEN** for the owner |
| **THE MOCKUP UI REFACTOR + THE CEO BOT'S LAST THREE BRIEFS** (owner briefs 2026-09-06). One card system taken from the approved "Case Command Center (V1)" mockup — its GRAMMAR, not its palette: a boxed section, an icon tile beside a bold name, facts as label/value rows, one filled primary over a two-across grid of outlined secondaries. Geometry is five `:root` tokens; the six families are composed **entirely from tokens this palette already had**, so not one hex literal entered the page and the anti-drift budget had nothing to catch. Applied to the phone Home strip, the case action row, the intake card and the CEO Bot's cards — **same doors, same `data-act`s, same conditions, same order** on all four. **The phone Home strip stopped swiping**: it was a 126px horizontal scroller showing two and a half of ten doors, and is a two-across grid showing all ten with no gesture and no overflow at 390 or 320 (`grid-auto-rows:1fr` is load-bearing — measured, all ten are 122px). **The desktop quick-tools row is byte-what-it-was.** The back-button audit found a real bug: `.close` is the dialog X's `position:absolute`, and the two Clients & Firms back buttons — not being `.casepage` — carried it at EVERY width; `.pagebar .close` is in flow now. `.uiact-lead` not `.lead` is the class-name contest for the sixth recorded time, caught before it shipped. Carried with it: the CEO Bot polish pass, the stacked ★ CEO / ✨ ASSISTANT pills on the thumb side, the Workflow-tab "Why?" cleanup, and the `notePrefUse` colon fix. | PR PENDING | ⏳ **CODED + TESTED** — awaiting merge |
| **THE CEO BOT IS WIRED** (owner brief 2026-09-06, from the staged plan — nothing redesigned). Two products, two doors, two colours: the Assistant stays navy/teal case OPERATIONS with the ✨ pill bottom-right; the CEO Bot is navy/GOLD portal/workflow ADVICE with its own rail button and a fab bottom-LEFT. **Read-only structurally** — two routes, and the only write is this user's own suggestion state inside the existing `user_pref` blob, so **no new table and no portal-setup dispatch**. Zero case-write verbs in the rendered drawer at either width, asserted. **THE COUNTERS WERE 400-ING IN PRODUCTION, SILENTLY**: `noteUse` sends `qt:` + the id and the allow-list had no colon, so every quick-action tap was refused behind that helper's own empty catch — and the counters that DID work would have made the quick actions look untouched, so the Unused Feature Watch would have recommended hiding controls the owner uses daily with *"you have not used this once"* as the evidence. **The gate cried wolf twice and both were the gate** (an invented intake selector; a signature drawn without `scrollIntoViewIfNeeded()`, which `test-intake.mjs` already records) — it reported the PRODUCT as failing to deliver an intake it had never managed to submit. **And its own self-check had to move above the summary derivation**: placed after it, its FAIL landed in `findings` while the printed total counted a snapshot taken before it — 0 FAIL, exit 0, real failure in the JSON. | #297 `e4434bc` | ✅ **DEPLOYED** — `deploy.yml` run #371 `success` and `deploy-portal.yml` run #132 `success`, both on `e4434bce`. Suites: worker **3731/0** · e2e **3306/0** · CEO gate **43 PASS/0/0** · intake **671/0** · deploy guard **92/0** · visitor **49/0**. **No schema change — no dispatch owed.** LIVE VERIFY **OPEN** for the owner. |
| **THE CEO CHARTER + THE CONSOLIDATED CLOSEOUT + THE WAY BACK** (owner briefs 2026-09-06; C1–C12 in `case-portal/CEO-CHARTER.md`). The closeout confirm CLOSES the case without the checklist — the owner's own overturn for this path ("'Case Ready' must NOT block 'Close Case'"), with a running day as the one blocker, refused naming who holds it. REQUESTED IS NOT COMPLETED: a `case_refund` row is only ever a refund completed outside the portal; requested keeps the money on the ledger and its word in `case_refund_status`; `/closeout/refund-done` is the owner's explicit completion. The statement wears the status and its settlement balance adds up on the client's own page. "Close Michelle's case" PREPARES (name resolved through the office's own search, exactly-one match, nothing executed). A closed case leaves Out-now/Reports-due — both arms had no admin status filter. Retainer paid / Close case on the case actions row; Close/archive intake with the reason as an admin note. MY PORTAL: `user_pref`, one row per signed-in user, /meta merge rule, factory hides (needs_assignment, lead_status), Settings → My Portal with reorder/hides/two resets, per-user tap metrics. | #296 `1fa7f4b` | ✅ **DEPLOYED** — `deploy.yml` run #370 `success` and `deploy-portal.yml` run #131 `success`, both on `1fa7f4b1`. Suites at merge: worker **3700/0** · e2e **3294/0** · intake **671/0** · deploy guard **92/0** · visitor **49/0**. **SCHEMA CHANGED (`case_closeout_detail`, `user_pref`) — `portal-setup.yml` dispatched on `master` after the merge.** LIVE VERIFY **OPEN** for the owner: the intake way-back links on a phone, Retainer paid / Close case on a private case, the no-work closeout (preset → requested refund → confirm), the closed-case summary, Settings → My Portal, and the Assistant's "Keep $500 non-refundable and note $500 refund requested". |
| **THE FINANCIAL CLOSEOUT — a refund is its own ledger event** (owner brief 2026-09-06 Parts 7–16; derived decisions D1–D9 in `case-portal/CLOSEOUT-FINANCIAL.md`). Two additive tables — `case_refund`, `case_closeout` — and four admin-only routes under `/cases/:no/`. **`retainer_payment` is never touched**: money received stays on the record forever, a refund is a separate event, every total is arithmetic over the two computed on read, and no total is stored anywhere. Three things the suite found while it was written: **two balances are not one question** (`final_balance` is the ledger's and reads $600 on a $1,000 case with $400 earned before a confirm, because the office really is still holding $600; `projected_balance` is what the agreed figures would settle to, and a projection drawn under the word FINAL is the portal asserting something that has not happened); **the statement prints CASE STATUS: CLOSED, so the case has to be closed** — the checklist can refuse a confirm, leaving a real refund on an open case, and the email would have told a client otherwise; and **the Assistant refused “refund the client $600” with a shrug** — nothing could ever have refunded anything, but the person asking got the ordinary help answer, so `refund` and `closeout` joined the blocked list. The Assistant PREPARES in the Daily Summary shape and proposes no split, because what the firm earned is not derivable from anything the portal holds. Page: the panel above the closing checklist (three states kept apart), `#fcdoc` as a seventh print region inside the ONE `@media print` block, and the intake screen carrying the intake's own EXISTING actions (owner live iPhone: the complaint was the distance). Not written to `send_log` — its `kind` CHECK is the non-idempotent rebuild `schema.sql` cannot do. | #295 `7b374fb` | ✅ **DEPLOYED** — `deploy.yml` run #369 `success` and `deploy-portal.yml` run #130 `success`, both on `7b374fb6`. Suites at merge: worker **3619/0** · e2e **3247/0** · intake **566/0** · deploy guard **92/0** · visitor **49/0**. **SCHEMA CHANGED — `portal-setup.yml` dispatched on `master` after the merge** (outcome recorded in the row below once the run finishes). LIVE VERIFY **OPEN** for the owner: the Case closeout panel on Billing & closing of a private case with a recorded retainer payment (form → review → record → statement → optional email), the intake screen's bottom action row, and the Assistant's "prepare the closeout". |
| **RATE-SHEET PARSER, PRINTABLE SHEET, AND ONE TAP TO THE SUBMITTED INTAKE** (owner brief 2026-09-06 Parts 2/3 + the live-iPhone intake correction). **The parser claims the non-refundable amount FIRST and removes it from the sentence** — two dollar figures, and the only thing telling them apart is that one stands next to the words "non-refundable"; take it out and whatever is left is the retainer, so the retainer pattern needs to know nothing about word order. All four of the owner's sentences pinned. **A loose match is safe because the flow makes it safe**: a seed that lands wrong is visible in the form, the preview and the confirmation before anything leaves. **Blank stays blank** — writing the default in would turn *whatever the standard is* into *this figure*. **`#rsdoc`** gives the rate sheet the print path it never had, the `#invdoc`/`#tldoc` pattern, with the Send control and Next-step panel OUTSIDE the region. **The intake card is the door**: a button carrying the identity straight to `details`, action row and Admin-status disclosure as SIBLINGS (a control inside a control is the Unit 40 nesting rule), *Review* renamed to what it opens, the lead ladder folded but untouched, SIGNED computed in SQL so the fact travels and the 50KB signature does not, and the back label finally saying where it already went. **Three rounds lost to traps this file already records**: backticks in a comment inside a SQL template literal; a comment naming the PDF header literal making itself the second occurrence; and my own assertion matching source casing against `innerText`, which returns text after `text-transform:uppercase` — the card was right and the test was wrong | #293 `f938e69` | ✅ **DEPLOYED** — `deploy.yml` run #367 **success** AND `deploy-portal.yml` run #129 **success**, both on `f938e69e`. Suites at merge: worker **3516/0** · e2e **3216/0** · intake **566/0** · deploy guard **92/0** · visitor **49/0**. **No schema change.** LIVE VERIFY **OPEN** |
| **THE PRIVATE RETAINER'S NON-REFUNDABLE PORTION** (owner brief 2026-09-05). The private sheet now states **Retainer / NON-REFUNDABLE PORTION (red) / 4-HOUR MINIMUM REQUIRED (gold)** plus one sentence — no percentage, no formula. **`nonRefundableFor` is the ONE source of truth** and the only thing that knows the rule: blank resolves to $500 and never disappears for being empty, a typed amount is used exactly, a retainer under the standard caps the portion at itself, and an amount above the retainer is **refused BY NAME** rather than clamped. **The page holds no default at all** — `/sheets` reports `non_refundable_default`, which also keeps the no-dollar-figure guard on `index.html` true. **PRIVATE ONLY, STRUCTURALLY**: the legal card is `{...priv}` plus a line, so the block is added to the private CARD where legal cannot inherit it — and the existing *legal card is the private pricing verbatim* pin passed unchanged, which is the evidence the placement is right. **A latent leak this unit activated and fixed:** the send wizard resolved its card by sheet id alone and three cards share `private_retainer`, so a LEGAL wizard had always been reading the PRIVATE card — harmless while only `name` was read off it, a leak the moment that card gained a block a law firm must not see. **The suite also refused a change I should not have made**: I added $1,000 to the retainer selector and *"the owner's four choices, standard first"* stopped it — left alone and raised with the owner instead. Colours measured: red **4.73:1**, gold **5.84:1** on the block's own ground (on `--neutral-bg` the red is 4.30 and would have failed). **There is no rate-sheet PDF in this portal and none was invented** — the email is the document, and preview and send are held together byte for byte | #291 `731fe7c` | ✅ **DEPLOYED** — `deploy.yml` run #365 **success** AND `deploy-portal.yml` run #128 **success**, both on `731fe7c7`. Suites at merge: worker **3498/0** (+27 checks: all seven of the owner's cases, the sub-$500 retainer, an explicit zero, and the private-only boundary from both directions) · e2e **3198/0** · intake **566/0** · deploy guard **92/0** · visitor **49/0**. Measured at 390px: the block is 312px wide with no sideways scroll; the wizard control is 16px type in a 44px box carrying the Worker's own figure as its placeholder. **No schema change, so no `portal-setup` was owed.** LIVE VERIFY **OPEN** |
| **THE THREE SENDS GO LIVE** (owner 2026-09-05: *"make it live / ai works it can take beta off also"*, scoped in the same exchange to rate sheet + intake link + payment options, with *"leave them refused"* on every other verb). **The architecture did not move:** each send is a level-3 `ASSISTANT_COMMANDS` row naming the ORDINARY route the desk's Send button posts to, the page dispatches to it, and the Assistant block still holds no SQL beyond `assistant_log` and calls `sendMail` NEVER. **The suite caught a real mistake** — I took the send verb OFF `ASSISTANT_BLOCKED`, which was wrong because the three carve-outs already RETURN above the list, so the entry only ever sees send-shaped sentences that are not one of the three; removing it turned *"email the firm their case documents"* from a named refusal into a shrug. Restored. **The source pin was REPLACED, not loosened**: sends are an allow-list of three exact route strings, each claimed by one command, every one level 3 and admin-only, no command routing to a delete/archive/void — and it parses the ROWS rather than grepping the prose, which retires the defect that made it fire on my own comments. Payment instructions gained the third workbench (`assistantPaymentPlan`, a pinned mirror asserted byte-identical to `emailPaymentOptions`). **You cannot send what you have not previewed**: Send posts `previewBody`, the exact object the preview rendered from, dropped on Edit and Back. Banner text moved into the Worker — one writer for a safety statement. No AI model is connected and none was added. **Two more gaps found reviewing my own diff, neither caught by a suite:** the preview and the send disagreed on the GREETING (a case-referenced intake goes through `/leads/:no/send-intake`, which greets with the lead's own name and ignores the body's — free while everything was a rehearsal, a broken promise once Send really sent; the mirror follows the sender now, asserted byte for byte), and **the pin claimed one route more than it could hold** — three exact routes while the page uses a fourth whenever a case is named. Nothing unsafe; the CLAIM was wider than the code, which is the failure a pin exists to prevent, committed by the pin | #288 `e859b6e` | ✅ **DEPLOYED** — `deploy.yml` run #362 **success** AND `deploy-portal.yml` run #127 **success**, both on `e859b6ec`. Suites at merge: worker **3471/0** (+28 new checks) · e2e **3172/0** · intake **566/0** · deploy guard **92/0** · visitor **49/0**. Measured at 390px: the preview's four actions 336×44 stacked, no horizontal overflow, header visible, Send clears the composer (565 vs 756). **No schema change, so no `portal-setup` was owed.** ✅ **LIVE VERIFIED by the owner, 2026-09-05 — *"works fine"*** against the list they were given: send a rate sheet for real from the Assistant and see it arrive and appear in the send history; the intake link and the payment instructions the same way; and the refusals still standing on *"email the client their case documents"*, *"delete this case"* and *"void that payment"*. **This is the current owner-verified checkpoint.** |
| **MOBILE WEB APP — Units A–F** (owner brief 2026-09-04, five approved mockups as the visual target). A phone bottom nav from `shell()` — which the case page and the field view do not use, so it cannot collide with their bars structurally. Then Home: **`QT` is ONE TABLE OF DOORS and `QT_DESK`/`QT_PHONE` are two orderings named beside it**, so the phone gets the owner's order (Rate Sheet first and the only card with the accent, then New/Private/Insurance/Law Firm Intake, then Reports & Packages) while the desktop row stays byte-identical — `dlabel` exists only so the desktop keeps saying *Intake a Client*, the drift the first build shipped silently. Both strips render and one is `display:none`, so exactly one is in the accessibility tree. Then an audit at 390px rather than a redesign: the rate sheet's fee lines were leaving the label **90px of a 312px panel** and now stack; **`Back to Cases` was drawn underneath the hamburger** at 390 and 768 (`.close` is the dialog X's rule and the class came with its position) — measured with `elementFromPoint`, desktop was never broken and is untouched, fixed at the 899px the burger already uses; and eleven controls computed under the 44px floor across a sweep of **all sixteen top-level screens**. The field view needed nothing. **`.pkg-b` was measured at 56px and its rule taken back out.** Page-only, no schema change | **Mobile Assistant scroll containment** (owner, real iPhone 2026-09-05). The sheet covered the page but a swipe inside it scrolled the PORTAL behind, and a long form could not be reached. **Measured at 390px first:** `.asst-work` was a 692px form in a 350px box, `.asst-log` 176px in 128px, both `overscroll-behavior: auto`, body `overflow: visible` — two competing scrollers that chained into the document. Three CSS answers, no scroll handlers: ONE scroller (`.asst-body`, `min-height:0` load-bearing) with `overscroll-behavior: contain`; a real backdrop that is a **sibling** of the panel, because `touch-action` is computed walking UP from the target and a wrapper would have disabled panning inside the sheet; and `overflow:hidden` on the document, which does not move the page so there is nothing to save or restore. Proven: panel scrolls, `window.scrollY` stays at 300 through the end and past it, closing returns it to exactly 300 and it scrolls again. Eight flows reach their bottom including the keyboard case. Desktop asserted untouched | #286 `580516a` | ✅ **DEPLOYED** — `deploy.yml` run #360 `success` on `580516a7` (page-only, so no Worker deploy applies). Suites at merge: worker **3434/0** · e2e **3154/0** · intake **566/0** · deploy guard **92/0** · visitor **49/0**. No schema change. LIVE VERIFY **OPEN** |
| **CASE COMMAND CENTER V1 COMPLETE** (owner briefs 2026-09-04 + 2026-09-05). Registry 2 → 5 — `start_day`, `end_day`, `add_task`, `complete_task` (level 2), `accept_intake` (level 3) — plus *cases ready to build* (the 19th phrase search could not answer), the case tabs as destinations, and the **payment PREFILL**: a review card whose button opens the case's EXISTING Record Payment form seeded through `RET_DRAFT`, carrying no command object so nothing can execute it. **The Daily Summary command deliberately writes no summary** — the builder's picks are the authorship — and a test reads the stored narrative back byte for byte after asking. The blocked list was not weakened for the prefill and it GREW: voiding and altering a payment were never on it. **Hardening: 22 adversarial assertions, all green on the first run.** Four things the tests found that I had wrong are fixed and written down, including two source pins that fired on my own comments — rewritten rather than loosened | #285 `c74027f` | ✅ **DEPLOYED** — site AND Worker both `success` on `c74027f7`. Suites at merge: worker **3434/0** · e2e **3108/0** · intake **566/0** · deploy guard **92/0** · visitor **49/0**. **No schema change, so no `portal-setup` was owed.** LIVE VERIFY **OPEN** for the owner |
| #284 `5adc64f` | ✅ **DEPLOYED** — `deploy.yml` run #358 `success` on `5adc64fe`. Page-only, so no Worker deploy applies and **no schema change, so no `portal-setup` was owed**. Suites at merge: worker **3333/0** · e2e **3080/0** · intake **566/0** · deploy guard **92/0** · visitor **49/0**. The e2e took four rounds and every failure was traced rather than waived (10+crash → 4 → 1 → 0); one was a REAL defect the suite found — the Overview thumbnail button collapses to a 16px control when its image fails to load — and the rest were assertions written for the single-strip world, each repointed at the property it actually protects. LIVE VERIFY **OPEN** for the owner: the Home quick-actions strip with Rate Sheet first, *Back to Cases* no longer under the hamburger, and the Assistant's Prepare a Rate Sheet form |
