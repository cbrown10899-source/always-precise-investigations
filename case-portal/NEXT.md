...
| Unit | PR / SHA | State |
| --- | --- | --- |
| **API Assistant Unit 5 + Unit 6 read half + §11 guide fix** — rate-sheet preparation + preview + SIMULATE (`ASSISTANT.md` A11; pinned mirror of `emailSheet`, byte-equality + refusal-mirror tests); live billing answers (A12; drafts excluded from outstanding; Unit 6's preparation half DEFERRED — owner question in A12); and the inert Explain & Guide Me toggle made real (found gap, `guide_intro`). No schema change | in flight on the branch | worker 3117/0 · e2e 2849/0 · deploy guard 86/0 · intake 558/0 · visitor 47/0 |
