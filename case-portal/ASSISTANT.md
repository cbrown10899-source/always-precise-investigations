...
| 5 | Rate-sheet preparation + preview + simulation (pricing via the real resolvers) | ✅ **MERGED** — #263 `099b817` (A11; byte-pins hold the mirror to the real sender; no schema change). Deploy verification recorded in NEXT.md |
| 6 | Invoice/billing read + preparation + simulation | **READ HALF MERGED** — #263 (A12) — live billing answers from `listInvoices`, drafts excluded from outstanding; preparation/simulation DEFERRED pending the owner's answer in A12 (no invoice-send route exists to rehearse; creating drafts would widen the Beta write pin) |
