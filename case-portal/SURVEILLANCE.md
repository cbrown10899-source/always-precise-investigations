...
| Phase | Status |
| --- | --- |
| 1. Foundation (route; reuse auth; verify case authorization server-side; shared data; Start button on eligible cases) | not started |
| 2. Active day (start day; persistent server-derived timer; end day with confirmation + totals; case-info drawer; remaining authorization) | not started |
| 3. Rapid reporting (quick activity; search; favorites; structured templates; one-tap No Change; editable narrative; save + Add Another; timeline) | not started |
| 4. Voice (device dictation everywhere; optional in-app voice entry with review + Use/Discard; never auto-submit; honest privacy wording; voice nav later/optional) | not started |
| 5. Evidence (photo/video upload; activity linking; mobile gallery; Dropbox large-video handoff prep) | not started |
| 6. Reporting (report builds from activities; mobile preview; submit → preserved version → admin review queue) | not started |
| 7. Web-app experience (bottom nav; drawer; PWA manifest/readiness; home-screen launch; resume active state) | not started |
| 8. Admin connection (Out Now with elapsed + last activity; reports awaiting review; Case Build handoff; invoice data) | not started |
| 9. Test (iPhone Safari; Android Chrome; desktop; lock/resume; refresh mid-day; uploads; permissions; back buttons; timer accuracy; report transfer) | not started |
