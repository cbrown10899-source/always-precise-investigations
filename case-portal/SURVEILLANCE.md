...
| 10. A top-level door, and pausing the day (owner, 2026-08-14) | **done** — 2026-08-14. Two things the owner found in real use, neither of which any ledger held. **The door:** `svLaunchButton()` rendered in exactly two places and both were a case's Overview tab, so from Safari on an iPad you had to open Cases, open a case and land on Overview before any way in existed — and `?surveillance=1` is the PWA start URL, which assumes the icon is already on the home screen. Both roles now carry an **Active surveillance** item in the navigation opening the same launcher, tested at 1112×834 and 390×844. The case-level button stays as the shortcut. **Pause:** `case_day_pauses` records spans server-side; elapsed is `(now - started) - closed spans`, and an open pause freezes the display on the server's `paused_at` — so nothing here stops a tick and the number is identical after a reload. One open pause per day is enforced by a partial unique index rather than a check in the route. Breaks come off the billable total, because `hours` is what authorization and invoices draw against, and the day-end message names the break instead of quietly returning a shorter day |
