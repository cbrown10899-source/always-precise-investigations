...
| Step | State | Evidence |
| --- | --- | --- |
| 9. Private lead card: Send Payment Options action | **CODED + TESTED** 2026-08-15 | `leadPayOpen` on the card, rendered only when the lead is not `claims`. E2E asserts a private and an insurance card **side by side on the same desk**: one offers it, the other does not, and the insurance card keeps its own two sends |
...
| 12. Standalone Send Payment Options dialog | **CODED + TESTED** 2026-08-15 | `paySendHtml()` — two screens, ask then preview, the same shape as the sheet wizard. `POST /payment-options/email` → `emailPaymentOptions()`, admin-only, writing `payment_send` with **`with_sheet: 0`** — the seam the column was added for, now used. `paymentOnlyEmail()` reuses `paymentBlockText/Html` rather than restating them, so the two renderings cannot drift |
...
| 16. Sent confirmation listing exactly what went | **DEPLOYED**, and now on both send paths | Worker returns `included{rate_sheet, intake, payment_methods}` `worker.js:934-939`; the page reads it back at `portal/index.html:6087-6094` and appends *"The retainer is still pending until you record it."* Read from the send record, not echoed from the form — as the reader's note below required. The standalone send does the same, and its answer also carries an explicit `retainer_marked_paid: false` |
