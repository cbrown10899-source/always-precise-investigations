...
| 7. Mobile (case home; bottom nav; activity/evidence/report/back flows) | **done** — 2026-08-13 (P15: an investigator's case opens on the FIELD HOME — the ring over the modules that are theirs (activity, report, photos, video — build and invoice never appear; `pkgProgress`/`pkgNextStep` gained a "field" scope, same functions, one flag per P24), one next step in field language ("Finish and submit the report", "Submitted — with the office") with GO, recent activity, then the assignment detail. On a phone the section bar becomes the fixed bottom navigation — short labels (Home · Activity · Evidence · Report for the field; Home · Field · Report · Admin for the office), safe-area padding, + Add activity pinned above it. Desktop keeps the top bar; both labels ride the same buttons and CSS picks.) |
| 8. Cleanup (remove duplicate legacy UI; move test controls; wire every button; routes; roles; desktop+mobile test) | **done** — 2026-08-13 (P20: Show example / Add a test case / Remove test cases moved off the case-list bar into **Settings → Developer & testing** (admin only; investigators get no Settings). The one Hide button stays over the list and the empty state still offers the example — teaching UI is not a test control. Adding a test case lands on the case list where it now sits; showing the example jumps there too. Duplicate-interface sweep: the phrase dropdown went in Phase 3, the inline sheet send in Phase 6, edit-in-preview single-homed in Phase 4 — nothing serves the same task twice. Desktop + a real 390×844 mobile pass ride the e2e. **This closes the UIBUILD handoff: all 8 phases shipped** (PRs #42–#48 + this one), suites worker 624 / portal 481 / intake 130 / alerts 41.) |
